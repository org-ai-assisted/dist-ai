...
if [ -z "${bindir}" ]; then
   for cand in \
      ...
      "/usr/bin" ; do
      if [ -x "${cand}/git-review-difftool" ]; then bindir="${cand}"; break; fi
   done
fi
...
r="${tmproot}/dt-meld"; make_change_repo "${r}" 'a\nb\n' 'a\nB\n'
...
r="${tmproot}/dt-kdiff3"; make_change_repo "${r}" 'a\nb\n' 'a\nB\n'
...
r="${tmproot}/dt-diff"; make_change_repo "${r}" 'a\nb\n' 'a\nB\n'
...
r="${tmproot}/dt-badutf8"; make_change_repo "${r}" 'clean\n' 'x\xff y\n'
...
r="${tmproot}/dt-binary"; make_change_repo "${r}" 'clean\n' 'a\x00b\n'
...
r="${tmproot}/dt-bidi"; make_change_repo "${r}" 'clean\n' 'x=1 //\xe2\x80\xae evil\n'
...
r="${tmproot}/dt-quiet"; make_change_repo "${r}" 'a\nb\n' 'a\nB\n'
: > "${r}/gui.log"
...
make_conflict_repo() {
   ...
   git -C "${dir}" config merge.verifySignatures false
   printf 'l1\nl2\nl3\n' > "${dir}/f"
   git -C "${dir}" add f; git -C "${dir}" commit --quiet --message init
   git -C "${dir}" switch --quiet --create feat
   printf 'l1\nFEAT\nl3\n' > "${dir}/f"
   git -C "${dir}" add f; git -C "${dir}" commit --quiet --message feat
   git -C "${dir}" switch --quiet main
   printf 'l1\nMAIN\nl3\n' > "${dir}/f"
   git -C "${dir}" add f; git -C "${dir}" commit --quiet --message main
...
r="${tmproot}/mt-meld"; make_conflict_repo "${r}"
...
r="${tmproot}/mt-kdiff3"; make_conflict_repo "${r}"
