...
- cwd: each emulator is launched from the harness's private `${HOME}` by `cd`-ing in
  the LAUNCHER, so a typed `cat hostile.log` resolves. Keep `cd` OUT of the rcfile
  (`.strc`) -- the shell inside secure-terminal sees a different `$HOME`, would walk
  away from the payload logs, and `cat hostile.log` would fail under a caption
...
- **Case A - random.** `head -c 1200 /dev/random`: genuine random data, no
  crafted escapes.
- **Case B - a crafted hostile log.** `hostile.log` carries, mid-stream, the
  ...
  short, deterministic `make-hostile-log.sh` (`./make-hostile-log.sh > hostile.log`).
- **Case C - homoglyph.** `homoglyph.log`: an install one-liner whose domain hides a
  Cyrillic look-alike (U+0430 for Latin a). Generated by `make-homoglyph-log.sh`.
