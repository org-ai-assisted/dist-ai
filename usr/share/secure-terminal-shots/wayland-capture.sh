...
##   --wayland-terminals   the native-Wayland set (default)
##   --x11-terminals       only xterm/urxvt/st (via labwc's Xwayland)
##   --all-terminals       both sets
...
## :0 and labwc's Xwayland cannot bind it (breaks the X11-only terminals).
if [ -z "${WLCAP_NS:-}" ]; then
...
mount -t tmpfs none /tmp/.X11-unix 2>/dev/null && chmod 1777 /tmp/.X11-unix
ip link set lo up 2>/dev/null || true

only=(); cases=(); settle=3; keep=0; set_choice=all
out_dir="$(pwd)/shots-wayland"
st_repo="${ST_REPO:-${HOME}/private-sources/secure-terminal}"
while [ "$#" -gt 0 ]; do
   case "$1" in
      --wayland-terminals) set_choice=wayland; shift ;;
      --x11-terminals)     set_choice=x11; shift ;;
      --all-terminals)     set_choice=all; shift ;;
      --only)  only+=("$2"); shift 2 ;;
      --case)  cases+=("$2"); shift 2 ;;
      --quick) only=(kitty); cases=(crafted); shift ;;
      --keep-running) keep=1; shift ;;
      --settle) settle="$2"; shift 2 ;;
      --out) out_dir="$2"; shift 2 ;;
      --st-repo) st_repo="$2"; shift 2 ;;
...
payload_cmd() {                          ## the command each terminal DISPLAYS
   case "$1" in
      crafted)   printf 'cat hostile.log' ;;
      homoglyph) printf 'cat homoglyph.log' ;;
      random)    printf 'head -c 1200 /dev/random' ;;   ## Kicksecure entropy guidance
   esac
}

## ---- labwc on the X11 backend (also gives us an Xwayland for the X11 set) -----
xvfb_pid=''; labwc_pid=''; xdisplay=':121'; xwl_display=''
start_compositor() {
   ...
   local before s i
   before=' '
   for s in "${XDG_RUNTIME_DIR}"/wayland-*; do [ -S "${s}" ] && before="${before}${s} "; done
   DISPLAY="${xdisplay}" WLR_BACKENDS=x11 WLR_X11_OUTPUTS=1 labwc >"${out_dir}/.labwc.log" 2>&1 &
   labwc_pid=$!
   for i in $(seq 1 40); do
      for s in "${XDG_RUNTIME_DIR}"/wayland-*; do
         [ -S "${s}" ] || continue; case "${s}" in *.lock) continue ;; esac
         case "${before}" in *" ${s} "*) : ;; *) WAYLAND_DISPLAY="$(basename -- "${s}")"; export WAYLAND_DISPLAY ;; esac
      done
      [ -n "${WAYLAND_DISPLAY:-}" ] && break; sleep 0.2
   done
   [ -n "${WAYLAND_DISPLAY:-}" ] || { echo "labwc: no wayland socket" >&2; tail -8 "${out_dir}/.labwc.log" >&2; return 1; }
   ...
   for i in $(seq 1 25); do
      ...
      [ -n "${xwl_display}" ] && break; sleep 0.3
   done
   echo "labwc up on ${xdisplay}: WAYLAND_DISPLAY=${WAYLAND_DISPLAY} xwayland=${xwl_display:-none}"
}

## ---- launch one terminal running `sh -c` from the log dir --------------------
launch_terminal() {
   local name="$1" cmd="$2"
   local sh="cd ${work}; printf 'user@host:~\$ %s\n' \"${cmd}\"; ${cmd}; exec sleep 600"
   ...
   case "${name}" in
      konsole)        "${wl[@]}" konsole --separate --hide-menubar --hide-tabbar -e sh -c "${sh}" >/dev/null 2>&1 & ;;
      qterminal)      "${wl[@]}" qterminal --hide-menu-bar -e "sh -c '${sh}'" >/dev/null 2>&1 & ;;
      xfce4-terminal) "${wl[@]}" xfce4-terminal --disable-server --hide-menubar -x sh -c "${sh}" >/dev/null 2>&1 & ;;
      mate-terminal)  "${wl[@]}" mate-terminal --disable-factory --hide-menubar -x sh -c "${sh}" >/dev/null 2>&1 & ;;
      alacritty)      "${wl[@]}" alacritty -o 'window.dimensions.columns=84' -o 'window.dimensions.lines=24' -e sh -c "${sh}" >/dev/null 2>&1 & ;;
      kitty)          "${wl[@]}" kitty -o remember_window_size=no -o initial_window_width=780 -o initial_window_height=520 sh -c "${sh}" >/dev/null 2>&1 & ;;
      secure-terminal)
         "${wl[@]}" PYTHONPATH="${st_repo}/usr/lib/python3/dist-packages" \
            python3 "${st_repo}/usr/bin/secure-terminal" -- bash -c "${sh}" >/dev/null 2>&1 & ;;
      xterm)          "${x[@]}" xterm -geometry 84x24 -e sh -c "${sh}" >/dev/null 2>&1 & ;;
      urxvt)          "${x[@]}" urxvt -geometry 84x24 -e sh -c "${sh}" >/dev/null 2>&1 & ;;
      st)             "${x[@]}" st -g 84x24 -e sh -c "${sh}" >/dev/null 2>&1 & ;;
   esac
}
...
capture_one() {
   ...
   if ! command -v convert >/dev/null; then mv "${outfile}.full.png" "${outfile}"; return 0; fi
   convert "${outfile}.full.png" -bordercolor black -border 1 -trim +repage "${outfile}" 2>>"${out_dir}/.grim.log"
   ...
   dim="$(identify -format '%w %h' "${outfile}" 2>/dev/null)"
   if [ -z "${dim}" ] || [ "${dim%% *}" -lt 20 ] || [ "${dim##* }" -lt 20 ]; then
      mv "${outfile}.full.png" "${outfile}"; return 2
   fi
   rm -f "${outfile}.full.png"
}

cleanup() { rm -rf "${work}"; [ "${keep}" = 1 ] && return; pkill -x labwc 2>/dev/null; kill "${labwc_pid}" "${xvfb_pid}" 2>/dev/null; }
trap cleanup EXIT

## ---- select the terminal set + run -------------------------------------------
start_compositor || exit 1
sleep 1
case "${set_choice}" in
   wayland) term_list=("${wayland_terminals[@]}") ;;
   x11)     term_list=("${x11_terminals[@]}") ;;
   all)     term_list=("${wayland_terminals[@]}" "${x11_terminals[@]}") ;;
esac
...
for name in "${term_list[@]}"; do
   for c in "${cases[@]}"; do
      ...
      sleep "${settle}"
      capture_one "${out_dir}/${name}.${c}.png"; rc=$?
      case "${rc}" in
         0) echo "captured ${name}.${c}" ;;
         2) echo "captured ${name}.${c} (WARN: window did not render -- full frame kept)" ;;
         *) echo "FAILED ${name}.${c}" >&2 ;;
      esac
