...
def _repo_file(relglob):
    if not REPO:
        pytest.fail("SECURITY_MISC_REPO is unset; suite is not wired to a checkout")
    matches = sorted(glob.glob(os.path.join(REPO, relglob)))
    if not matches:
        pytest.fail(f"expected exactly one match for {relglob} under {REPO}, found none")
    if len(matches) > 1:
...
def _client_audit(policy, ssh_config):
    ...
    if not _wait_listen(port):
        ...
        pytest.fail("ssh-audit client-audit listener did not come up")
...
def _start_sshd(tmp_path, port, directives):
    """Run an unprivileged sshd on a loopback high port; return the Popen."""
    sshd = "/usr/sbin/sshd"
    ...
    proc = subprocess.Popen(
        [sshd, "-D", "-f", config_path, "-E", os.path.join(tmp_path, "sshd.log")]
    )
    if not _wait_port(port):
        proc.terminate()
        log = ""
        try:
            with open(os.path.join(tmp_path, "sshd.log"), encoding="ascii") as handle:
                log = handle.read()
        except OSError:
            pass
        pytest.fail(f"test sshd did not come up on 127.0.0.1:{port}\n{log}")
    return proc
...
def _server_config_algos():
    return _parse_ssh_config_algos(
        _repo_file("etc/ssh/sshd_config.d/30_security-misc.conf*")
    )
...
def _client_config_algos():
    return _parse_ssh_config_algos(
        _repo_file("etc/ssh/ssh_config.d/30_security-misc.conf*")
    )
...
def test_server_config_passes_policy(tmp_path, server_config_algos):
    ...
    _require("ssh-audit")
    tool = _repo_file("usr/bin/ssh-audit-test*")
    policy = _repo_file("usr/share/security-misc/ssh-audit/server.policy*")
    port = _free_port()
    proc = _start_sshd(str(tmp_path), port, _server_directives(server_config_algos))
    try:
...
def test_stock_server_fails_policy(tmp_path):
    ...
    proc = _start_sshd(str(tmp_path), port, {})
    try:
        result = subprocess.run(
            ["bash", tool, "--policy", policy, "--port", str(port), "--", "127.0.0.1"],
            capture_output=True,
...
def test_client_config_passes_policy():
    ...
    client_config = _repo_file("etc/ssh/ssh_config.d/30_security-misc.conf*")
    policy = _repo_file("usr/share/security-misc/ssh-audit/client.policy*")
...
def test_policy_matches_server_config(server_config_algos):
    ...
    policy = _parse_policy_algos(
        _repo_file("usr/share/security-misc/ssh-audit/server.policy*")
    )
...
def test_policy_matches_client_config(client_config_algos):
    ...
    policy = _parse_policy_algos(
        _repo_file("usr/share/security-misc/ssh-audit/client.policy*")
    )
    assert [a for a in policy["key exchanges"] if a not in KEX_PSEUDO] == \
