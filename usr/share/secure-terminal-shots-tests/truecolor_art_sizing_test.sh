...
## Regression: st_size_art_payload (comparison-capture.sh) turns the live viewport geometry in
...
## real-GUI shot cannot regression-test offline: field order (ROWS then COLS), the 2/3-rows
...
## Extract st_size_art_payload verbatim between the sentinels the source carries for this test.
fn="$(sed -n '/## BEGIN st_size_art_payload/,/## END st_size_art_payload/p' "${src}")"
case "${fn}" in
   *'st_size_art_payload() {'*)
      ;;
   *)
      printf '%s\n' 'FAIL: could not extract st_size_art_payload (BEGIN/END sentinels moved?)' >&2
      exit 1
      ;;
esac
eval "${fn}"
...
expect() {  ## $1=label $2=want_width $3=want_rows
   local label="$1" ww="$2" wr="$3" gr gw meas rest
   if [ ! -s "${HOME}/art.payload" ]; then
      gr=0
      gw=0
   else
      meas="$(python3 "${measure}" "${HOME}/art.payload")"
      gr="${meas%% *}"
      rest="${meas#* }"
      gw="${rest%% *}"
   fi
...
## Good geometry: "24 130" is ROWS=24 COLS=130 -> art sized 130 cols x (24*2/3)=16 rows.
## A field-order bug (COLS then ROWS) would size 24 cols x 86 rows -- caught here.
seed_fallback
printf '%s\n' '24 130' > "${HOME}/.st_geom"
st_size_art_payload
expect 'good geom 24x130 -> 130 cols x 16 rows' '130' '16'
...
## Degenerate viewport (rows so small art_rows = rows*2/3 < 2) -> fallback preserved.
seed_fallback
printf '%s\n' '2 130' > "${HOME}/.st_geom"
st_size_art_payload
...
printf '%s\n' 'OK: st_size_art_payload sizes to the live viewport and falls back safely'
