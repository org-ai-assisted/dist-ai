...
Known, already-reported defects (org-ai-assisted/pyte-audit) are listed in
``known_crashes.json`` next to this file, keyed by finding id; the harness
treats a crash whose signature is in that allowlist as expected and keeps
going. A finding the tree under test declares fixed (``pyte_audit_fixes``) is
dropped from the allowlist, so its return is gated like any new defect rather
than tolerated forever. Any crash with a NEW signature -- or any invariant
violation -- fails the run (exit 1) and prints the seed and the exact
reproducing sequence, so it can be turned into a regression test.
...
def _load_allowlist() -> set[str]:
    """Signatures to tolerate: the audited defects this tree has NOT fixed."""
    try:
        with open(ALLOWLIST_PATH, encoding='utf-8') as handle:
            by_finding = json.load(handle)['signatures']
    except (OSError, ValueError, KeyError):
        return set()
    fixed = pyte_audit_fixes.declared_fixes(pyte)
    return {signature
            for finding_id, signatures in by_finding.items()
            for signature in signatures
            if finding_id.upper() not in fixed}
