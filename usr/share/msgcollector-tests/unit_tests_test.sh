...
msgcollector_src=""
for candidate in \
  "${MSGCOLLECTOR_REPO:-}/usr/libexec/msgcollector" \
  "${dm_checkout}/packages/kicksecure/msgcollector/usr/libexec/msgcollector" \
  "/usr/libexec/msgcollector"; do
  if [ -d "${candidate}" ] && [ -r "${candidate}/check" ]; then
    msgcollector_src="${candidate}"
    break
  fi
done
