...
## Integration tests for the msgcollector message-collection pipeline: drives
## the REAL scripts end-to-end (CLI collection, type files, progress, popup
## queues, pv_wrapper, sanitization) against an isolated XDG_RUNTIME_DIR.
##
...
## /usr/libexec/msgcollector, i.e. the installed package). dist-ai-tests-all
## exports MSGCOLLECTOR_REPO and HELPER_SCRIPTS_PATH at the checkouts, so the
## sourced msgcollector_shared resolves helper-scripts without any install.
...
test_msgdispatcher_dispatch_x_argparse() {
  ...
  local result
  result=0
  "${msgcollector_libexec}/msgdispatcher_dispatch_x.py" "invalid_type" "title" "msg" "0" 2>/dev/null || result=$?
  if [ "${result}" != "0" ]; then
...
test_generic_gui_message_argparse() {
  local result
  result=0
  "${msgcollector_libexec}/generic_gui_message.py" "invalid_type" "title" "msg" "" "ok" 2>/dev/null || result=$?
  if [ "${result}" != "0" ]; then
...
test_generic_gui_message_argparse_button() {
  local result
  result=0
  "${msgcollector_libexec}/generic_gui_message.py" "info" "title" "msg" "" "invalid_button" 2>/dev/null || result=$?
  if [ "${result}" != "0" ]; then
...
test_one_time_popup_argparse_missing() {
  local result
  result=0
  "${msgcollector_libexec}/one-time-popup.py" 2>/dev/null || result=$?
  if [ "${result}" != "0" ]; then
