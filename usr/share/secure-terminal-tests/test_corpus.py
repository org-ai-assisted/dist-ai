...
def assert_all_paths(name, text):
    ...
    # the TUI cell path (pyte grid), per character, in EVERY mode. tui_cell always
    # returns a string (the grapheme, the box placeholder, or a space) -- never None,
    # so `cell is not None` alone was vacuous (see the tui-cell canary). Assert the
    # real property this path exists to guarantee: no dangerous code point survives
    # into the returned cell.
    for ch in text:
        for mode in MODES:
            cell = S.tui_cell(ch, mode)
            ok(not any(ord(c) in DANGEROUS_CPS for c in cell),
               '%s/%s: tui_cell let a dangerous code point through for %r'
               % (name, mode, ch))
...
# CANARY for the tui_cell path in assert_all_paths: tui_cell always returns a string
# (grapheme / box placeholder / space), never None, so the pre-fix `cell is not None`
# could never fail. Pin that a dangerous code point is neutralized (not returned raw)
# while `is not None` proves nothing.
_tccan = S.tui_cell(chr(0x202E), 'box')
ok(_tccan is not None and not any(ord(c) in DANGEROUS_CPS for c in _tccan),
   'tui-cell canary: a dangerous code point is neutralized, not returned raw '
   '(is-not-None alone was vacuous)')
