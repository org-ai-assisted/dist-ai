...
def _reset_screen_spec(screen):
    ...
    cursor.attrs / margins / tabstops / savepoints / DECCOLM-width clears the direct
    assignment would otherwise miss."""
    ...
    screen.tabstops = set(range(8, screen.columns, 8))
    screen.savepoints.clear()
    if screen.saved_columns is not None:
...
def t10_reset_baseline():
    ...
    sp = _armed_screen('\x1b(0\x1b7')                       # G0 special-graphics, DECSC saved
    _reset_screen_spec(sp)
    pyte.Stream(sp).feed('\x1b8')                           # DECRC with the savepoint cleared
    if sp.savepoints or sp.g0_charset is not pyte.charsets.LAT1_MAP:
        fail('T10: reset left a DECSC savepoint; ESC 8 re-selected the charset')
    dc = _armed_screen('\x1b[?3h')                          # DECCOLM widened to 132
    prior = dc.saved_columns
    _reset_screen_spec(dc)
    if dc.saved_columns is not None or dc.columns != prior:
        fail('T10: reset did not undo the DECCOLM grid width')
    return checked
...
def t10_canaries():
    ...
    # A reset that forgets the tab stops (leaves them cleared) is caught.
    s4 = _armed_screen('\x1b[3g')                     # TBC: every stop cleared, not restored
