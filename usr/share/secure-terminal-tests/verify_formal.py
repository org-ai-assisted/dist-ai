...
together the WHOLE pure sanitizer, input AND output):
  ...
  T10 COMMAND-HOOK SUBMIT-GATE INTEGRITY -- the CLI command_hook path. INV-LEAK:
      the hook reads the typed line ONLY at a bare prompt, never a foreground
      program's keystrokes (a sudo/ssh password). INV-GATE: a command reaches the
      shell ONLY via an `allow` verdict or an explicit user `run`; a declined block
      is never submitted, and no disarmed-SIGINT bypass lets the NEXT Enter submit
      it unjudged. Proved over an ABSTRACT Z3 model of the gate, then the REAL
      terminal.py SOURCE is pinned to that model by AST lemmas (guard at every hook
      call site; the decline path keeps the fail-safe flag; the mirrored-line erase
      writes Backspaces only, never a SIGINT). The Qt runtime stays in test_widget.py.
...
METHOD, and what is PROVED vs ASSUMED (honest scope):
  ...
  * STRUCTURAL LEMMA, stated and executable-checked, not machine-proved:
  ...
  does not model Qt, pyte, the pty, or terminal.py's RUNTIME -- except T10, which
  proves the command-hook submit gate over an abstract Z3 model and pins it to
  terminal.py's SOURCE STRUCTURE (AST, no import); the widget's live behaviour
  stays covered by test_widget.py.
...
# ===========================================================================
# T10 -- COMMAND-HOOK SUBMIT-GATE INTEGRITY (the CLI command_hook path).
#
# The command_hook judges a shell command line before Enter submits it. Two
# security properties, both regressions the reviewers found, pinned here:
#   INV-LEAK  -- the hook reads the typed line ONLY at a bare prompt. With a
#                foreground program the keystrokes are ITS input (a sudo/ssh
#                password); routing them to the external hook process leaks them.
#   INV-GATE  -- a command reaches the shell ONLY through an `allow` verdict or an
#                explicit user `run`; a declined block is never submitted, and its
#                post-state cannot let the NEXT Enter submit it unjudged (the bypass
#                codex found via `stty intr undef`).
#
# terminal.py is the Qt widget, not modeled at runtime here (that is test_widget.py's
# job). This proves the property over an ABSTRACT model of the submit gate with Z3,
# then pins the REAL terminal.py SOURCE to that model with AST lemmas (parsed as
# text, no Qt import): the guard is present at EVERY hook call site, the decline path
# never clears the fail-safe flag, and the mirrored-line erase emits Backspaces only
# (no SIGINT -- so no tcflush that would swallow a suggestion, no signal to a child).
# ===========================================================================
import ast as _ast


def _terminal_source():
    """terminal.py's text, located from the (Qt-free) package dir and read as text
    so this proof never imports the widget."""
    import os
    path = os.path.join(os.path.dirname(S.__file__), 'terminal.py')
...
def _is_self_attr(node, attr):
    return (isinstance(node, _ast.Attribute) and node.attr == attr
            and isinstance(node.value, _ast.Name) and node.value.id == 'self')


def _is_not_foreground(node):
    """A `not self.has_foreground_program()` guard operand."""
    return (isinstance(node, _ast.UnaryOp) and isinstance(node.op, _ast.Not)
            and _is_self_call(node.operand, 'has_foreground_program'))
...
def _failsafe_violations(source):
    ...
    if dirty_if is None:
        return violations + ['the `if self._line_dirty:` fail-safe branch is gone']
    inner = [n for n in _ast.walk(dirty_if) if isinstance(n, _ast.If)
             and _compares_action_run(n.test)]
    if not inner:
        return violations + ['the dirty branch no longer forks on action == run']
    for node in inner:
        if _assigns_false(node.orelse, '_line_dirty'):    # the decline path
            violations.append('the unknown-line DECLINE path clears _line_dirty '
                              '(reopens the stty-intr-undef bypass)')
    return violations


def _erase_violations(source):
    """The counted erase emits Backspaces ONLY: _clear_typed_line writes exactly once
    and every bytes literal in it is 0x7F -- no CR/LF (a submit byte), no 0x03 (SIGINT
    / tcflush). A source-level audit backing the Z3 alphabet proof."""
    tree = _ast.parse(source)
    fn = _func_def(tree, '_clear_typed_line')
    if fn is None:
        return ['_clear_typed_line not found (source drift?)']
    violations = []
    writes = [n for n in _ast.walk(fn) if _is_self_call(n, '_write')]
    if len(writes) != 1:
        violations.append('must write exactly once (found %d)' % len(writes))
    for node in _ast.walk(fn):
        if isinstance(node, _ast.Constant) and isinstance(node.value, bytes):
            for byte in node.value:
                if byte != 0x7F:
                    violations.append('writes a non-Backspace byte 0x%02X '
                                      '(want 0x7F only)' % byte)
    return violations


def t10_source_ast():
    """Discharge the three structural lemmas against the REAL terminal.py source."""
    source = _terminal_source()
    for msg in _guard_violations(source):
        fail('T10 INV-LEAK: ' + msg)
    for msg in _failsafe_violations(source):
        fail('T10 INV-GATE: ' + msg)
    for msg in _erase_violations(source):
...
def t10_z3_gate():
    ...
    z3_prove('T10-GATE-no-next-bypass',
             z3.Implies(declined, z3.Not(next_unjudged)), assumptions=[z3.Not(fg)])


# Crafted BROKEN sources -- each must trip exactly one structural lemma.
_CANARY_UNGUARDED = (
    'class W:\n'
    '    def keyPressEvent(self, e):\n'
    '        if self._hook is not None and self._hook_intercept():\n'
    '            return\n')
_CANARY_DECLINE_CLEARS = (
    'class W:\n'
...
def t10_canaries():
    ...
    _expect_caught('T10/next-bypass', not z3_prove(
        ...
        assumptions=[z3.Not(fg)], report=False))
    # Z3 canary: a model that submits on a block-decline breaks submit-needs-permit.
    broken_submit = z3.Or(submit, z3.And(z3.Not(dirty_in), nonempty, z3.Not(allow),
                                         z3.Not(run)))
    _expect_caught('T10/submit-permit', not z3_prove(
        'canary-T10-submit', z3.Implies(broken_submit, z3.Or(allow, run)),
        assumptions=[z3.Not(fg)], report=False))
    # Z3 canary: a model that reads the command even under a foreground program.
    broken_reads = z3.And(z3.Not(dirty_in), nonempty)              # no Not(fg) guard
    _expect_caught('T10/leak', not z3_prove(
        'canary-T10-leak', z3.Implies(fg, z3.Not(broken_reads)), report=False))
    # AST canaries: each broken source must trip its structural lemma.
...
def main():
    ...
    sys.stdout.write('  T10   command-hook submit gate: Z3 leak/no-bypass model '
                     '+ terminal.py AST lemmas ...\n')
    t10_z3_gate()
    t10_source_ast()
