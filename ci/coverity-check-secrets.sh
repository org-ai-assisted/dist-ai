...
if [ -z "${COVERITY_TOKEN:-}" ]; then
  printf '%s\n' '::error::COVERITY_SCAN_TOKEN repository secret is not set. See the reusable coverity workflow header for setup.' >&2
  exit 1
fi

if [ -z "${COVERITY_EMAIL:-}" ]; then
  printf '%s\n' '::error::COVERITY_SCAN_EMAIL repository secret is not set. See the reusable coverity workflow header for setup.' >&2
  exit 1
fi

if [ -z "${COVERITY_PROJECT:-}" ]; then
  printf '%s\n' '::error::COVERITY_PROJECT repository variable is not set. See the reusable coverity workflow header for setup.' >&2
  exit 1
fi
