...
## header and at least one "DRY-RUN: clone" line. Match UNANCHORED: github-org-
## clone emits the header via 'log notice', i.e. "[NOTICE]: <n> repos to process
## under <dir>", so a '^' anchor can never match. Consistent with the canonical
## github-org-tools-tests dry-run assertion ("repos to process under").
if ! grep --quiet --fixed-strings -- 'repos to process under' <<< "${out}"; then
   printf '%s\n' 'FAIL: expected "N repos to process under" header in output' >&2
   exit 1
fi
